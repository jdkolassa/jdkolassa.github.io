...
<template #slides>
  ...
  >
    <swiper-slide
      ...
      :key="position"
      class="w-1/2 job rounded-2xl text-white border-cyan-400 border border-double flex flex-col sm:flex-row mx-2 p-4"
    >
      ...
      <div>
        <span class="font-sans">{{ position.location }}</span
        >&nbsp;&nbsp;<img
          src="../assets/point_24x24.png"
          class="inline"
        />&nbsp;&nbsp;<span class="font-sans">{{ position.dates }}</span>
        <h1 class="heading">{{ position.title }}</h1>
        <div class="flex flex-row sm:flex-col">
          <i
            v-for="skill in position.skills"
            class="fab lg:text-2xl sm:text-sm md:text-md justify-self-center"
            v-bind:class="`${skill}`"
          />
        </div>
      </div>
